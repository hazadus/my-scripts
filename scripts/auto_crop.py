...
def crop_image(input_path):
    ...
    try:
        ...
        with Image.open(input_path) as image:
            print(
                f"Загружено изображение: {image.size[0]}x{image.size[1]} ({image.mode})"
            )
            ...
            centered_bbox = get_centered_crop_box(
                bbox, original_size, center_of_mass, padding=20
            )
            new_left, new_top, new_right, new_bottom = centered_bbox
            new_size = (new_right - new_left, new_bottom - new_top)

            print(
                f"Центрированные границы: ({new_left}, {new_top}) -> ({new_right}, {new_bottom})"
            )
            ...
            if image.format == "PNG":
                ...
                if image.mode in ("RGBA", "LA") or "transparency" in image.info:
                    save_kwargs["transparency"] = (
                        "transparency" if "transparency" in image.info else None
                    )
            ...
            print(
                f"📊 Размер уменьшен с {original_size[0]}x{original_size[1]} до {new_size[0]}x{new_size[1]}"
            )
...
def main():
    ...
    parser.add_argument("image_path", type=str, help="Путь к PNG файлу для обрезки")
